...
n_tam = 300;                  % noise, nT
b_true = zeros(N, 3);
b_true(1, :) = [5000; 3000; 6000]';  % Constant bias, nT
...
x_true = [b_true(1, :)'; D_true(1,1); D_true(2,2); D_true(2,2); D_true(1, 2); D_true(1, 3); D_true(2, 3)];

for i = 1:N-1
    b_true(i + 1, :) = b_true(i, :) + n_tam * randn(1, 3) * dt;
    B_tam(i, :) = ((eye(3) + D_true) \ (dcms(:, :, i) * B_true(i, :)' + b_true(i, :)' + n_tam * randn(3, 1)))';
end
...
subplot(3, 1, 1)
plot(T, b_true(:, 1) - x(:, 1), "DisplayName", "b_1 (nT)", "Color", "#ff3366", "LineWidth", 2.5)
ylabel("b_1 Error (nT)")
...
subplot(3, 1, 2)
plot(T,  b_true(:, 2) - x(:, 2), "DisplayName", "\theta (^o)", "Color", "#ff7f11", "LineWidth", 2.5)
ylabel("b_2 Error (nT)")
ylim([-abs(max(x_err(:, 2))), abs(max(x_err(:, 2)))])
grid on
subplot(3, 1, 3)

plot(T, b_true(:, 3) - x(:, 3), "Color", "#011627", "LineWidth", 2.5)
ylabel("b_3 Error (nT)")
